...
calc_mec_exp2 <- function(model, x_var, ale_Js, tol, var_Js, plotting)
{
   num_sample <- 5
   MEC <- c()
   #currently fitting too well...
   #there is no way for modifying the goodness of the fit
   ...
   for (v in x_var){
      ...
      while(Rsq_j < 1 - tol){
         ...
         while(length(errs) < num_sample){
            tryCatch({seg_j = segmented(obj = lm_j, seg.Z = ~ x, npsi = K, control = seg.control(tol = 0.1, seed = sample.int(99999, 1)))
                        seg_fit_j <- fitted(seg_j)
                        Rsq_j <- summary(lm(y~seg_fit_j))$adj
                     },
                     finally = next)
            
            # seg_j = tryCatch(segmented(obj = lm_j, seg.Z = ~ x, npsi = K, control = seg.control(tol = 0.1, seed = 12345)),
            #                  error = function(e) {return(segmented(obj = lm_j, seg.Z = ~ x, npsi = K, control = seg.control(tol = 0.1, seed = 123)))},
            #                  finally = {print("shit")})
            errs <- c(errs, Rsq_j)
         }
         
         K = K+1
         k_errs = c(k_errs, mean(errs))
      }
      
      K = which(k_errs == max(k_errs - (1-tol)))
      
      if(plotting){
         j <- which(v == x_var)
         ds_plot <- data.frame(seg_fit_j, y, x)
         ds_plot <- ds_plot[order(ds_plot$x),]
         file.i <- paste0(paste(output.dir,"/Figures_EXP/",sep = ""), model,"/MEC","_",j,".pdf")
         pdf(file.i)
         plot(seg_fit_j~x,pch = 20,type = "l", data = ds_plot,
              ylab = "ALE", xlab = expression(x[j]), lwd = 2)
         lines(y ~ x, data = ds_plot,col = 3)
         grid(10)
         dev.off()
      }
   ...
   MEC_model1 <- sum(MEC*var_Js/sum(var_Js))
...
calc_mec_exp <- function(model, x_var, ale_Js, tol, var_Js, plotting)
{
   ...
   for (v in x_var){
      ...
      MEC_try_function <- function(ale_j) {
         ...
         cont = seg.control(tol = 0.1)
         
         #segmented(lm_j, seg.Z = ~ x, control = x)
         
         seg_j = selgmented(olm = lm_j,type = 'bic', Kmax = 10, msg = FALSE, return.fit = TRUE, control = cont)#, seg.Z = ~ x, type = 'bic', msg = FALSE)
         #K = seg_j$n.psi
         
         #cont = seg.control(tol = 0.00001, fix.npsi = F)
         #seg_j = segmented(obj = lm_j, seg.Z = ~ x, control = cont)
         
         K = length(seg_j$id.psi.group)
   ...
   MEC_model1 <- sum(MEC1*var_Js/sum(var_Js))
   #MEC_model2 <- sum(MEC2*var_Js/sum(var_Js))
   
   #cat("exp mec1: ",MEC_model1)
   #cat("exp mec2: ",MEC_model1)
